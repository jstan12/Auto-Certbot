...
ssh $USER@$DAG_URL "[ "$(docker ps -a | grep access-gateway)" ] && docker stop access-gateway;certbot renew;sudo cp $leDir/privkey.pem $homeDir/privkey.pem;sudo cp $leDir/cert.pem $homeDir/cert.pem"
